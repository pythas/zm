...
pub const GlobalRenderState = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, gctx: *zgpu.GraphicsContext) !Self {
        ...
        const texture_size = 32 * 32 * 4;

        var textures = try allocator.alloc(u8, 1 * texture_size);
        defer allocator.free(textures);

        const texture = try Texture.init(allocator, "assets/ship.png");

        const offset = 0 * texture_size;
...
pub const WorldRenderer = struct {
    ...
    pub fn createChunkRenderData(self: Self, chunk: *Chunk) !void {
        ...
        const chunk_data = try self.allocator.alloc(u8, Chunk.chunkWidth * Chunk.chunkHeight);
        ...
        self.gctx.queue.writeTexture(
            ...
            .{ .bytes_per_row = Chunk.chunkWidth, .rows_per_image = Chunk.chunkHeight },
...
pub const SpriteRenderer = struct {
    ...
    pub fn draw(self: Self, pass: zgpu.wgpu.RenderPassEncoder, global: *const GlobalRenderState) void {
        ...
        const bind_group = gctx.lookupResource(global.bind_group).?;
